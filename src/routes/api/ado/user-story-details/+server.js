...
// GET handler for User Story details
export async function GET({ url, fetch }) {
    ...
    try {
        // Get user story details with additional fields
        const fields = [
            'System.Id',
            'System.Title',
            'System.State',
        ...
        const path = `${projectId}/_apis/wit/workitems/${userStoryId}?fields=${fields.join(',')}&api-version=${AZURE_DEVOPS_CONFIG.apiVersion}`;
        ...
        return json({ error: error.message || 'Failed to fetch User Story details' }, { status: 500 });
    }
} 